...
function extractMatchFromRegExp(value, regexp, group = 0, defaultValue = null) {
  if (value) {
...
function combineStrings(delimiter = "", ...strings) {
...
class RequestApi {
  ...
  constructor() {
    ...
    this.$cache = new cacheClock.CacheClock({
      autoStart: false,
      debug: false
    });
  }
...
class UrbexHeaders {
  $headers = {};
  constructor(headers, withDefaults = true) {
    if (withDefaults) {
  ...
  static construct(headers = {}, withDefaults = true) {
...
function convertStringToURIComponent(input, urlMount = "") {
...
function parseURIIntoComponent(component, allowEndpoints = true) {
...
function serializeParams(params, serializerType = "string") {
...
class RequestConfig {
  ...
  parseIncomingConfig(config, allowEndpoints) {
    var _configuration$timeou, _configuration$maxCon;
    ...
    const timeout = parseInt(((_configuration$timeou = configuration.timeout) === null || _configuration$timeou === void 0 ? void 0 : _configuration$timeou.toString()) ?? DEFAULT_CLIENT_OPTIONS.timeout.toString());
    if (isNaN(timeout)) {
      configuration.timeout = DEFAULT_CLIENT_OPTIONS.timeout;
    }
    const maxContentLength = parseInt(((_configuration$maxCon = configuration.maxContentLength) === null || _configuration$maxCon === void 0 ? void 0 : _configuration$maxCon.toString()) ?? DEFAULT_CLIENT_OPTIONS.maxContentLength.toString());
  ...
  merge(config) {
    ...
    const incomingHeaders = ((_config$headers = config.headers) === null || _config$headers === void 0 ? void 0 : _config$headers.get()) ?? {};
